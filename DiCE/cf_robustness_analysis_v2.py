...
#     # # Complete grid search: all combinations of n_estimators and max_depth for each model type
#     # n_estimators_values = [50, 100, 150, 200]
#     # max_depth_values = [3, 4, 5, 6]
#     # model_types = ['random_forest', 'xgboost', 'lightgbm']
    
#     # # Generate all 16 combinations for each model type (16 × 3 = 48 total for tree-based models)
#     # for model_type in model_types:
#     #     for n_estimators in n_estimators_values:
#     #         for max_depth in max_depth_values:
#     #             model_perturbations.append((model_type, max_depth, n_estimators))
...
#             # Save counterfactuals to CSV
#             save_counterfactuals_to_csv(cf_list, "DiCE", "Spambase", fold, "baseline")
...
#             # COMMENTED OUT: Data perturbation analysis (already completed)
...
#             # Data perturbations commented out - continue with model perturbations
...
#!/usr/bin/env python3
"""
Counterfactual Robustness Analysis (v2) — Corrected & Hardened

- Full model perturbation grid including AdaBoost depths 3/4/5/6 × n_estimators 50/100/150/200
- Explicit environment logging (script path, scikit-learn version)
- Robust error handling with tracebacks
- AdaBoost estimator/base_estimator compatibility
- Extra debug logs to verify what actually ran
"""
...
import matplotlib.pyplot as plt
import seaborn as sns  # noqa: F401 (not used, but harmless)

# DiCE ML imports
import dice_ml
from dice_ml.utils import helpers  # noqa: F401 (may be useful elsewhere)
...
def generate_counterfactuals(x_test, model, dice_data, method='random', total_cfs=2):
    ...
    for i in range(len(x_test)):
        ...
        try:
            ...
            if len(cf_result) > 0:
                cf_values = cf_result.iloc[0].values
                cf_class = cf_values[-1]
                cf_features = cf_values[:-1]
                cf_row = list(cf_features) + [cf_class, True]
                cf_list.loc[i] = cf_row
                successful_cfs += 1
            else:
                default_row = list(query_instance.iloc[0].values) + [0, False]
                cf_list.loc[i] = default_row
                failed_cfs += 1
        except Exception:
            default_row = list(query_instance.iloc[0].values) + [0, False]
    ...
    success_rate = successful_cfs / len(x_test)
    log_print("Counterfactual generation complete:")
    log_print(f"  Successful: {successful_cfs}/{len(x_test)} ({success_rate:.2%})")
    log_print(f"  Failed: {failed_cfs}/{len(x_test)} ({(1-success_rate):.2%})")
    return cf_list, success_rate
...
def calculate_comprehensive_metrics(model, cf_list, x_test, x_train):
    """
    Calculate validity, distances, and LOF score
    """
    # Only successful CFs
    successful_mask = cf_list['success'] == True
    if successful_mask.sum() == 0:
    ...
    # Remove the aux columns for prediction
    cf_features = successful_cfs.drop(['success', 'cf_class'], axis=1)
    ...
    try:
        combined_data = np.vstack([x_train.values, cf_features.values])
        lof = LocalOutlierFactor(n_neighbors=50, contamination=0.1)
        lof_labels = lof.fit_predict(combined_data)
        cf_lof_labels = lof_labels[-len(cf_features):]
        avg_lof_score = float(np.mean(cf_lof_labels))
    except Exception as e:
        log_print(f"Warning: Could not calculate LOF scores: {e}")
        avg_lof_score = 0.0
    ...
    return {
        'validity': validity,
        'flipped': int(flipped),
        'total': int(len(successful_cfs)),
        'l2_distance': float(avg_l2_distance),
        'l0_distance': float(avg_l0_distance),
        'lof_score': float(avg_lof_score)
    }
...
def main():
    ...
    log_print("=" * 80)
    log_print("COUNTERFACTUAL ROBUSTNESS ANALYSIS (v2) — Corrected")
    log_print("=" * 80)
    log_print(f"[LOG] Logging session to: {log_filename}")
    log_print(f"[TIME] Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    log_print(f"[SCRIPT] Running: {os.path.abspath(__file__)}")
    try:
        from sklearn import __version__ as sklver
        log_print(f"[ENV] scikit-learn: {sklver}")
    except Exception:
        log_print("[ENV] scikit-learn: <unknown>")
    ...
    # Data perturbations (kept for completeness; currently commented downstream)
    data_perturbations = [
    ...
    # -----------------------------
    # FULL GRID incl. AdaBoost depths
    # -----------------------------
    from itertools import product
    n_estimators_values = [50, 100, 150, 200]
    max_depth_values = [3, 4, 5, 6]
    ...
    for fold in range(5):
        ...
        try:
            ...
            # DiCE setup (Spambase: continuous features)
            log_print(f"Setting up DiCE framework for fold {fold}...")
            train_data_with_label = train_processed.copy()
            continuous_features = X_train.columns.tolist()
            dice_data = dice_ml.Data(
            ...
            log_print(f"  Baseline L0 distance: {baseline_metrics['l0_distance']:.2f}")
            log_print(f"  Baseline LOF score: {baseline_metrics['lof_score']:.4f}")
            log_print(f"    Bin 0: Remove 0% -> validity: {baseline_metrics['validity']:.4f}, accuracy: {test_acc:.4f}, "
                      f"L2: {baseline_metrics['l2_distance']:.4f}, L0: {baseline_metrics['l0_distance']:.2f}, LOF: {baseline_metrics['lof_score']:.4f}")
            ...
            #             )
            #         elif model_type == 'adaboost':
            #             # Explicit AdaBoost debug line
            #             log_print(f"[DBG] AdaBoost base tree depth = {max_depth}, n_estimators = {n_estimators}")
            #             from sklearn.ensemble import AdaBoostClassifier
            #             from sklearn.tree import DecisionTreeClassifier
            #             base_tree = DecisionTreeClassifier(max_depth=max_depth, random_state=42)
            #             try:
            ...
            #             'fold': fold
            #         })
            #         log_print(f"  {model_type} ({max_depth}, {n_estimators}): validity: {cf_metrics['validity']:.4f}, "
            #                   f"accuracy: {model_test_acc:.4f}, L2: {cf_metrics['l2_distance']:.4f}, "
            #                   f"L0: {cf_metrics['l0_distance']:.2f}, LOF: {cf_metrics['lof_score']:.4f}")
    ...
    baseline_validity_mean = np.mean(all_fold_results['baseline_validity']) if all_fold_results['baseline_validity'] else 0.0
    baseline_validity_std = np.std(all_fold_results['baseline_validity']) if all_fold_results['baseline_validity'] else 0.0
    baseline_success_mean = np.mean(all_fold_results['baseline_success_rate']) if all_fold_results['baseline_success_rate'] else 0.0
    baseline_success_std = np.std(all_fold_results['baseline_success_rate']) if all_fold_results['baseline_success_rate'] else 0.0
    baseline_accuracy_mean = np.mean(all_fold_results['baseline_model_accuracy']) if all_fold_results['baseline_model_accuracy'] else 0.0
    baseline_accuracy_std = np.std(all_fold_results['baseline_model_accuracy']) if all_fold_results['baseline_model_accuracy'] else 0.0
    baseline_l2_mean = np.mean(all_fold_results['baseline_l2_distance']) if all_fold_results['baseline_l2_distance'] else 0.0
    baseline_l2_std = np.std(all_fold_results['baseline_l2_distance']) if all_fold_results['baseline_l2_distance'] else 0.0
    baseline_l0_mean = np.mean(all_fold_results['baseline_l0_distance']) if all_fold_results['baseline_l0_distance'] else 0.0
    baseline_l0_std = np.std(all_fold_results['baseline_l0_distance']) if all_fold_results['baseline_l0_distance'] else 0.0
    baseline_lof_mean = np.mean(all_fold_results['baseline_lof_score']) if all_fold_results['baseline_lof_score'] else 0.0
    baseline_lof_std = np.std(all_fold_results['baseline_lof_score']) if all_fold_results['baseline_lof_score'] else 0.0
    ...
    log_print(f"CF Validity: {baseline_validity_mean:.4f} ± {baseline_validity_std:.4f}")
    log_print(f"CF L2 Distance: {baseline_l2_mean:.4f} ± {baseline_l2_std:.4f}")
    log_print(f"CF L0 Distance: {baseline_l0_mean:.2f} ± {baseline_l0_std:.2f}")
    log_print(f"CF LOF Score: {baseline_lof_mean:.4f} ± {baseline_lof_std:.4f}")
    log_print(f"Individual fold model accuracies: {[f'{acc:.4f}' for acc in all_fold_results['baseline_model_accuracy']]}")
    log_print(f"Individual fold CF success rates: {[f'{rate:.4f}' for rate in all_fold_results['baseline_success_rate']]}")
    log_print(f"Individual fold CF validities: {[f'{val:.4f}' for val in all_fold_results['baseline_validity']]}")

    # Data perturbation summary (if used)
    log_print("\nDATA PERTURBATION ROBUSTNESS SUMMARY:")
    log_print("-" * 150)
    log_print(f"{'Perturbation':<15} {'Bin':<5} {'Data':<12} {'Mean Validity':<13} {'Std Validity':<12} "
              f"{'Mean Accuracy':<13} {'Std Accuracy':<12} {'Mean L2':<10} {'Mean L0':<10} {'Validity Δ':<11} {'Per-Fold Validities'}")
    log_print("-" * 150)
    ...
    for perturb_type, bins_data in all_fold_results['data_perturbations'].items():
        ...
        for bin_num, fold_results in bins_data.items():
            if fold_results:
                ...
                mean_validity = float(np.mean(validities))
                std_validity = float(np.std(validities))
                mean_accuracy = float(np.mean(accuracies))
                std_accuracy = float(np.std(accuracies))
                mean_l2 = float(np.mean(l2_distances))
                mean_l0 = float(np.mean(l0_distances))
                validity_delta = mean_validity - baseline_validity_mean
                ...
                per_fold_str = [f'{v:.3f}' for v in validities]
                log_print(f"{perturb_type:<15} {bin_num:<5} {data_description:<12} {mean_validity:<13.4f} {std_validity:<12.4f} "
                          f"{mean_accuracy:<13.4f} {std_accuracy:<12.4f} {mean_l2:<10.4f} {mean_l0:<10.2f} {validity_delta:<+11.4f} {per_fold_str}")

    # Model perturbation summary
    log_print("\nMODEL PERTURBATION ROBUSTNESS SUMMARY:")
    log_print("-" * 150)
    log_print(f"{'Model Configuration':<30} {'Mean Validity':<13} {'Std Validity':<12} {'Mean Accuracy':<13} "
              f"{'Std Accuracy':<12} {'Mean L2':<10} {'Mean L0':<10} {'Validity Δ':<11} {'Per-Fold Validities'}")
    log_print("-" * 150)
    ...
    for model_key, fold_results in all_fold_results['model_perturbations'].items():
        if fold_results:
            ...
            per_fold_str = [f'{v:.3f}' for v in validities]
            log_print(f"{model_key:<30} {mean_validity:<13.4f} {std_validity:<12.4f} {mean_accuracy:<13.4f} "
                      f"{std_accuracy:<12.4f} {mean_l2:<10.4f} {mean_l0:<10.2f} {validity_delta:<+11.4f} {per_fold_str}")

    # Quick completeness check by model type
    from collections import Counter
    counts = Counter(k.split('_')[0] for k, v in all_fold_results['model_perturbations'].items() if v)
    log_print(f"[CHECK] Completed configs per model type: {dict(counts)}")
    ...
    if len(all_fold_results['baseline_validity']) > 0:
        ...
        # Average change per data perturb type
        data_avg_changes = {}
        for perturb_type, bin_results in data_summary_results.items():
            if bin_results:
                perturbed_results = [res for bin_num, res in bin_results.items()
                                     if not (perturb_type in ['minor_deletion', 'major_deletion'] and bin_num == 0)]
                if perturbed_results:
                    data_avg_changes[perturb_type] = float(np.mean([res['validity_delta'] for res in perturbed_results]))

        # Average change per model type
        model_avg_changes = {}
        mtypes_present = set(key.split('_')[0] for key in model_summary_results.keys())
        for mtype in mtypes_present:
            type_results = [res for key, res in model_summary_results.items() if key.startswith(mtype)]
            if type_results:
                model_avg_changes[mtype] = float(np.mean([res['validity_delta'] for res in type_results]))
        ...
        if data_avg_changes:
            ...
            log_print(f"• Most robust to: {most_robust_data[0]} (avg validity change: {most_robust_data[1]:+.4f})")
            log_print(f"• Least robust to: {least_robust_data[0]} (avg validity change: {least_robust_data[1]:+.4f})")
            data_robustness_score = 1 - float(np.mean([abs(v) for v in data_avg_changes.values()]))
            log_print(f"• Overall data perturbation robustness score: {data_robustness_score:.4f} (higher is better)")
        ...
        if model_avg_changes:
            ...
            log_print(f"• Most robust to: {most_robust_model[0]} (avg validity change: {most_robust_model[1]:+.4f})")
            log_print(f"• Least robust to: {least_robust_model[0]} (avg validity change: {least_robust_model[1]:+.4f})")
            model_robustness_score = 1 - float(np.mean([abs(v) for v in model_avg_changes.values()]))
            log_print(f"• Overall model perturbation robustness score: {model_robustness_score:.4f} (higher is better)")
    ...
    total_experiments = data_exp_count + model_exp_count + 5
    log_print(f"\n[SUMMARY] EXPERIMENT SUMMARY:")
    log_print(f"• Total experiments across all folds: {total_experiments}")
    log_print(f"• Baseline experiments: 5 (one per fold)")
    log_print(f"• Data perturbation experiments: {data_exp_count}")
