#!/usr/bin/env python3
"""
Counterfactual Robustness Analysis (NICE v3) - German Credit Dataset
...
This version uses the NICE algorithm for counterfactual generation and the German Credit dataset (mixed categorical and numerical features).
...
def setup_logging():
    ...
    log_filename = f"cf_robustness_analysis_nice_v3_{timestamp}.log"
...
def prepare_german_credit_features(x_train):
    """
    Prepare German Credit features for NICE
    Based on the notebook: cat_feat = [1, 3, 8], num_feat = [0, 7, 2, 4, 5, 6]
    Feature names: ['Age', 'Sex', 'Job', 'Housing', 'SavingAccounts', 'CheckingAccount', 'CreditAmount', 'Duration', 'Purpose']
    ...
    log_print(f"German Credit feature names: {feature_names}")
    
    # Based on the notebook analysis and German Credit dataset structure
    # Categorical features: Sex, Housing, Purpose (indices depend on column order)
    # Numerical features: Age, Job, SavingAccounts, CheckingAccount, CreditAmount, Duration
    
    cat_feat = []
    num_feat = []
    
    for i, feature in enumerate(feature_names):
        if feature.lower() in ['sex', 'housing', 'purpose']:
            cat_feat.append(i)
        else:
            num_feat.append(i)
    
    # If we can't identify features by name, use the notebook's indices as fallback
    if len(cat_feat) == 0 and len(feature_names) >= 9:
        # From notebook: cat_feat = [1, 3, 8] for ['Age', 'Sex', 'Job', 'Housing', 'SavingAccounts', 'CheckingAccount', 'CreditAmount', 'Duration', 'Purpose']
        cat_feat = [1, 3, 8]  # Sex, Housing, Purpose
        num_feat = [0, 2, 4, 5, 6, 7]  # Age, Job, SavingAccounts, CheckingAccount, CreditAmount, Duration
...
def main():
    ...
    log_print("=" * 80)
    log_print("COUNTERFACTUAL ROBUSTNESS ANALYSIS (NICE v3) - GERMAN CREDIT DATASET")
    log_print("=" * 80)
    ...
    for fold in range(5):
        ...
        try:
            ...
            save_counterfactuals_to_csv(cf_list, "NICE", "German-Credit", fold, "baseline")
            ...
            if not args.skip_data_perturbation:
                ...
                # Generate CFs with 50% data using NICE (German Credit dataset specific)
    ...
    try:
        ...
        log_print("\nBASELINE PERFORMANCE ACROSS ALL FOLDS (NICE - German Credit):")
        log_print("-" * 80)
        ...
        log_print("\nDATA PERTURBATION ROBUSTNESS SUMMARY (NICE - German Credit):")
        log_print("-" * 150)
        ...
        log_print("\nMODEL PERTURBATION ROBUSTNESS SUMMARY (NICE - German Credit):")
        log_print("-" * 150)
        ...
        plt.title('NICE Counterfactual Explanation Robustness Across Data Perturbations (German Credit)\n5-Fold Cross-Validation with Error Bars', fontsize=14)
        ...
        plt.tight_layout()
        data_plot_filename = f"cf_data_robustness_nice_german_5fold_plot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    ...
    log_print("\n🏛️ GERMAN CREDIT DATASET INSIGHTS (NICE):")
    log_print(f"• Dataset contains heterogeneous features for credit risk assessment")
