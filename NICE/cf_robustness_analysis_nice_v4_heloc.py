#!/usr/bin/env python3
"""
Counterfactual Robustness Analysis (NICE v4) - HELOC Dataset
...
This version uses the NICE algorithm for counterfactual generation and the HELOC dataset (mixed categorical and numerical features).
...
def setup_logging():
    ...
    log_filename = f"cf_robustness_analysis_nice_v4_heloc_{timestamp}.log"
...
def prepare_heloc_features(x_train):
    """
    Prepare HELOC features for NICE
    HELOC dataset typically has features related to credit risk with mixed types
    ...
    log_print(f"HELOC feature names: {feature_names}")
    
    # HELOC dataset analysis for categorical vs numerical features
    cat_feat = []
    num_feat = []
    ...
    for i, feature in enumerate(feature_names):
        ...
        # Heuristic: if feature has <= 10 unique values and dtype is int/float, treat as categorical
        # Otherwise treat as numerical
        if unique_vals <= 10 and x_train[feature].dtype in ['int64', 'float64']:
            cat_feat.append(i)
        else:
            num_feat.append(i)
    
    # If no categorical features identified, assume all are numerical (typical for HELOC)
    if len(cat_feat) == 0:
...
def main():
    ...
    log_print("=" * 80)
    log_print("COUNTERFACTUAL ROBUSTNESS ANALYSIS (NICE v4) - HELOC DATASET")
    log_print("=" * 80)
    ...
    for fold in range(5):
        ...
        try:
            ...
            cat_feat, num_feat = prepare_heloc_features(X_train)
            ...
            save_counterfactuals_to_csv(cf_list, "NICE", "HELOC", fold, "baseline")
    ...
    try:
        ...
        log_print("\nBASELINE PERFORMANCE ACROSS ALL FOLDS (NICE - HELOC):")
        log_print("-" * 80)
        ...
        log_print("\nDATA PERTURBATION ROBUSTNESS SUMMARY (NICE - HELOC):")
        log_print("-" * 150)
        ...
        log_print("\nMODEL PERTURBATION ROBUSTNESS SUMMARY (NICE - HELOC):")
        log_print("-" * 150)
        ...
        plt.title('NICE Counterfactual Explanation Robustness Across Data Perturbations (HELOC)\n5-Fold Cross-Validation with Error Bars', fontsize=14)
        ...
        plt.tight_layout()
        data_plot_filename = f"cf_data_robustness_nice_heloc_5fold_plot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    ...
    log_print("\n🏦 HELOC DATASET INSIGHTS (NICE):")
    log_print(f"• Dataset contains financial features for credit risk assessment")
    log_print(f"• HELOC (Home Equity Line of Credit) dataset for loan default prediction")
