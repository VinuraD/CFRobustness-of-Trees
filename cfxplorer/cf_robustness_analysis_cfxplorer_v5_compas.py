#!/usr/bin/env python3
"""
Counterfactual Robustness Analysis (CFXplorer v5) - COMPAS Dataset
...
This version uses the CFXplorer algorithm for counterfactual generation and the COMPAS dataset (categorical and numerical features).
...
def setup_logging():
    ...
    log_filename = f"cf_robustness_analysis_cfxplorer_v5_compas_{timestamp}.log"
...
def create_comprehensive_visualizations(all_results, output_dir="cfxplorer_plots"):
    ...
    fig.suptitle('CFXplorer Counterfactual Data Robustness Analysis - COMPAS Dataset', fontsize=16, fontweight='bold')
...
def print_statistical_summary(all_results):
    ...
    log_print(f"  • CFXplorer handles criminal justice predictive features effectively")
    log_print(f"  • Mixed categorical/numerical COMPAS features provide diverse CF scenarios")  
    log_print(f"  • Criminal justice context requires careful CF interpretation")
...
def main():
    ...
    log_print("="*80)
    log_print("COUNTERFACTUAL ROBUSTNESS ANALYSIS (CFXPLORER v5) - COMPAS DATASET")
    log_print("="*80)
