#!/usr/bin/env python3
"""
Counterfactual Robustness Analysis (FeatureTweak v2) - Spambase Dataset
...
This version uses the FeatureTweak algorithm for counterfactual generation and the Spambase dataset (continuous features only).
...
def setup_logging():
    ...
    log_filename = f"cf_robustness_analysis_featuretweak_v2_{timestamp}.log"
...
def run_model_perturbations(X_train, y_train, X_test, y_test, baseline_cf_list, fold_idx):
    ...
    # Define model configurations to test (FeatureTweak works with tree-based models)
    model_configs = []
    ...
    for model_type, params in model_configs:
        try:
            ...
            log_print(f"      {model_type} {params}: validity {metrics['validity']:.4f}, accuracy {accuracy:.4f}")
...
def create_comprehensive_visualizations(all_results, output_dir="featuretweak_plots"):
    ...
    fig.suptitle('FeatureTweak Counterfactual Data Robustness Analysis - Spambase Dataset', fontsize=16, fontweight='bold')
...
def print_statistical_summary(all_results):
    ...
    log_print(f"  • FeatureTweak leverages tree structure for interpretable CFs")
    log_print(f"  • Continuous features allow for precise epsilon-satisfactory instances")  
    log_print(f"  • Tree-based models provide clear decision paths for tweaking")
    log_print(f"  • Email classification benefits from actionable feature modifications")
...
def main():
    ...
    log_print("="*80)
    log_print("COUNTERFACTUAL ROBUSTNESS ANALYSIS (FEATURETWEAK v2) - SPAMBASE DATASET")
    log_print("="*80)
    ...
    try:
        ...
        log_print(f"\n✅ FeatureTweak robustness analysis completed successfully!")
