#!/usr/bin/env python3
"""
Counterfactual Robustness Analysis (FeatureTweak v4) - HELOC Dataset
...
This version uses the FeatureTweak algorithm for counterfactual generation and the HELOC dataset (numerical features only).
...
def setup_logging():
    ...
    log_filename = f"cf_robustness_analysis_featuretweak_v4_heloc_{timestamp}.log"
...
def create_comprehensive_visualizations(all_results, output_dir="featuretweak_plots"):
    ...
    fig.suptitle('FeatureTweak Counterfactual Data Robustness Analysis - HELOC Dataset', fontsize=16, fontweight='bold')
...
def print_statistical_summary(all_results):
    ...
    log_print(f"  • FeatureTweak optimized for pure numerical financial features")
    log_print(f"  • Epsilon adjustments enable precise continuous feature modifications")  
    log_print(f"  • HELOC financial domain benefits from tree-based actionable insights")
    log_print(f"  • Numerical features provide smooth counterfactual transitions")
...
def main():
    ...
    log_print("="*80)
    log_print("COUNTERFACTUAL ROBUSTNESS ANALYSIS (FEATURETWEAK v4) - HELOC DATASET")
    log_print("="*80)
