#!/usr/bin/env python3
"""
Counterfactual Robustness Analysis (FeatureTweak v3) - German Credit Dataset
...
This version uses the FeatureTweak algorithm for counterfactual generation and the German Credit dataset (mixed categorical/numerical features).
...
def setup_logging():
    ...
    log_filename = f"cf_robustness_analysis_featuretweak_v3_{timestamp}.log"
...
def create_comprehensive_visualizations(all_results, output_dir="featuretweak_plots"):
    ...
    fig.suptitle('FeatureTweak Counterfactual Data Robustness Analysis - German Credit Dataset', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(f'{output_dir}/cf_data_robustness_german_5fold_plot_{timestamp}.png', 
...
def print_statistical_summary(all_results):
    ...
    log_print(f"  • FeatureTweak handles mixed categorical/numerical features effectively")
    log_print(f"  • Tree-based epsilon adjustments work well with credit scoring features")  
...
def main():
    ...
    log_print("="*80)
    log_print("COUNTERFACTUAL ROBUSTNESS ANALYSIS (FEATURETWEAK v3) - GERMAN CREDIT DATASET")
    log_print("="*80)
