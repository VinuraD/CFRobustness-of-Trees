#!/usr/bin/env python3
"""
Counterfactual Robustness Analysis (CEML v5) - COMPAS Dataset
...
The COMPAS dataset contains heterogeneous features (categorical and numerical).
...
def setup_logging():
    ...
    log_filename = f"cf_robustness_analysis_ceml_v5_compas_{timestamp}.log"
...
def prepare_compas_features(df):
    """
    Prepare COMPAS features for CEML, handling categorical and numerical features
...
def main():
    ...
    log_print("=" * 80)
    log_print("COUNTERFACTUAL ROBUSTNESS ANALYSIS (CEML v5) - COMPAS DATASET")
    log_print("=" * 80)
    ...
    for fold in range(5):
        ...
        try:
            ...
            continuous_features, categorical_features = prepare_compas_features(train_processed)
            ...
            save_counterfactuals_to_csv(cf_list, "CEML", "COMPAS", fold, "baseline")
    ...
    log_print("\n" + "=" * 80)
    log_print("COUNTERFACTUAL ROBUSTNESS INSIGHTS (COMPAS DATASET - CEML - 5 FOLDS)")
    log_print("=" * 80)
    ...
    log_print("\n⚖️ COMPAS DATASET INSIGHTS (CEML):")
