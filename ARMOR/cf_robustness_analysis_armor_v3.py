#!/usr/bin/env python3
"""
Counterfactual Robustness Analysis (ARMOR v3) - German Credit Dataset
...
This version uses the ARMOR algorithm for counterfactual generation and the German Credit dataset.
Note: ARMOR handles mixed-type data (categorical + continuous) using HEOM distance.
"""
...
def setup_logging():
    ...
    log_filename = f"cf_robustness_analysis_armor_v3_{timestamp}.log"
...
def create_comprehensive_visualizations(all_results, output_dir="armor_plots"):
    ...
    fig.suptitle('ARMOR Counterfactual Data Robustness Analysis - German Credit Dataset', fontsize=16, fontweight='bold')
...
def print_statistical_summary(all_results):
    ...
    log_print(f"\n[KEY INSIGHTS] KEY INSIGHTS FOR GERMAN CREDIT DATASET:")
    log_print(f"  - ARMOR handles mixed-type data (categorical + continuous)")
    log_print(f"  - HEOM distance used for heterogeneous features")
    log_print(f"  - Immutable features (e.g., Sex) are respected during generation")


def parse_args():
    parser = argparse.ArgumentParser(description='ARMOR CF Robustness Analysis - German Credit')
...
def main():
    ...
    log_print("="*80)
    log_print("COUNTERFACTUAL ROBUSTNESS ANALYSIS (ARMOR v3) - GERMAN CREDIT DATASET")
    log_print("="*80)
